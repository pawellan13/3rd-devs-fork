...
export class TextSplitter {
    ...
    async split(text: string, limit: number): Promise<IDoc[]> {
        ...
        while (position < totalLength) {
            ...
            const { chunkText, chunkEnd } = this.getChunk(
                text,
                position,
                limit
            );
            ...
            const { content, urls, images } =
                this.extractUrlsAndImages(chunkText);
    ...
    private getChunk(
        ...
        const overhead =
            this.countTokens(this.formatForTokenization("")) -
            this.countTokens("");
        ...
        let end = Math.min(
            start +
                Math.floor(
                    ((text.length - start) * limit) /
                        this.countTokens(text.slice(start))
                ),
            text.length
        );
