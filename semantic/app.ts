...
const data = ["Music Player", "Todo List", "Email Client", "Web Browser"];
const queries = [
    "Play Music",
    "Add to my todo-list ...",
    "Check my emails",
    "Search web for https://example.com",
    "Open a new tab medialan.pl",
    "Next song",
];
...
async function initializeData() {
    const points = await Promise.all(
        data.map(async (text) => {
            const doc = await textSplitter.document(text, "gpt-4", {
                role: "embedding-test",
            });
            return doc;
        })
